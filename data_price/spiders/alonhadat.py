...
class AlonhadatSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        timestamp = self.extract_timestamp(response)
    ...
    def extract_timestamp(self, response):
        start_date = self.extract(response, '#left > div.property > div.title > span', 'start_date')
        now = date.today().strftime("%d/%m/%Y")
        timestamp = (date.today() - date.fromisoformat(start_date)).days
        return timestamp
